...
var opts struct {
	Input  string `short:"i" long:"input" description:"Path to the Reshade preset ini" required:"true"`
	Output string `short:"o" long:"output" description:"Path to output the vkBasalt configuration to"`
}
...
func main() {
	...
	reshadePresetAbsolute, err := filepath.Abs(opts.Input)
	if err != nil {
		log.Fatal(err)
	}
	reshadePresetPath := filepath.Dir(reshadePresetAbsolute)
	// name of the reshade preset directory
	reshadePresetName := strings.TrimSuffix(filepath.Base(opts.Input), filepath.Ext(opts.Input))
	// name of new basalt preset, reshade name + _vkBasalt
	var basaltPresetPath string
	if len(opts.Output) > 0 {
		basaltPresetPath = opts.Output
	} else {
		basaltPresetPath = filepath.Dir(reshadePresetPath) + "/" + reshadePresetName + "_vkBasalt"
	}
	basaltPresetFile := basaltPresetPath + "/" + reshadePresetName + ".conf"

	// load the reshade prefix ini
	reshadePresetConfig, err := ini.Load(opts.Input)
	if err != nil {
		log.Fatal(err)
	}
